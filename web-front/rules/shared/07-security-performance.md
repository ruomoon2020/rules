...
1. 列表、详情、搜索建议、弹窗、打印、导出和错误提示必须使用同一数据分级与脱敏策略；不得只在列表脱敏。
2. 复制到剪贴板、下载、打印和生成分享链接属于数据出站动作，须校验权限、记录敏感操作，并避免把明文长期留在浏览器历史或本地缓存。
3. DOM、HTML 注释、前端缓存、埋点和错误上报中不得保留“页面上隐藏但源码可见”的敏感值。
4. 水印只能作为追责辅助，不能替代鉴权、数据范围和脱敏。
5. 浏览器兼容矩阵、CSP 指令和允许的外部域写入 `99-project-local` 或项目安全基线；未声明时不得虚构支持范围。
