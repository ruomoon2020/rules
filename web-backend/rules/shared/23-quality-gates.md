...
| 依赖安全 | OWASP Dependency-Check / SCA / license check | 高危漏洞必须阻断 |
...
1. PR 至少运行编译、单测、静态检查、契约检查（若接口变更）。
2. DB 变更必须跑 Flyway validate；多库项目至少 MySQL + PostgreSQL 各跑一次迁移。
3. 生产发布分支必须运行集成测试或 Testcontainers 等价用例。
4. 跳过门禁必须写明原因、风险、owner 与补跑计划。
5. GitHub Actions 样板见 `examples/ci/github-actions-backend.yml`；接入后按项目 Maven / Gradle、数据库和契约路径调整。
