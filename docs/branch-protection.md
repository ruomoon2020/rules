...
本仓提供的 Required Checks 来源见：[`./.github/workflows/validate-rules-packages.yml`](../.github/workflows/validate-rules-packages.yml)。
...
- 规则接入门禁：`python scripts/check-project-adoption.py --repo . --stack backend --strict`
...
- 规则接入门禁：`python scripts/check-project-adoption.py --repo . --stack frontend --strict`
...
- 安全门禁：secret scan
- 供应链门禁：`supply-chain-required`
- 规则接入门禁：`python scripts/check-project-adoption.py --repo . --stack miniapp --strict`
